...
WeibullRandomVariable::WeibullRandomVariable(double scale, double shape, double location)
{
        shape_ = shape;
        scale_ = scale;
	location_ = location;
...
WeibullRandomVariable::WeibullRandomVariable(double scale, double shape, RNG* rng) 
{
        shape_ = shape;
        scale_ = scale;
        rng_ = rng;
...
{
        return(rng_->weibull(scale_, shape_, location_));
}
...
double WeibullRandomVariable::avg()
{
   // this is NOT the mean, but the median
   // TODO return the mean
   return ( location_ + scale_ * pow (0.69314718, 1.0/shape_));
}  
    
 
/*
 * GBAR Random Variable
 * by Andrea Angelini, Marco Cavicchioni, Vito D'Eri, Domenico Fiorentini, 
 *	Riccardo Proietti, Gianfranco Santoro.
 *
 *"A Gamma-Based Framework for Modeling Variable-Rate MPEG Video Sources: The GOP
 * GBAR Model" - M. Frey, S. Nguyen-Quang - IEEE/ACM TRANSACTION ON NETWORKING,
 * Vol. 8, NO.6, DEC 2000
 *
 */
static class GBAR_RandomVariableClass : public TclClass {
public:
	GBAR_RandomVariableClass() : TclClass("RandomVariable/GBAR") {}
	TclObject* create(int, const char*const*) {
		return(new GBAR_RandomVariable());
	}
} class_gbarranvar;

GBAR_RandomVariable::GBAR_RandomVariable()
{
	bind("alpha_", &alpha_);
	bind("rho_",&rho_);
	first = 1;
...
GBAR_RandomVariable::GBAR_RandomVariable(double alpha, double rho)
{
	alpha_ = alpha;
	rho_ = rho;
	first = 1;
...
void GBAR_RandomVariable::setScale(double scale)
{
	alpha_ = scale;
}

void GBAR_RandomVariable::setShape(double shape)
{
	rho_ = shape;
}

double GBAR_RandomVariable::avg()
{
	//TODO
	return 0;
}

double GBAR_RandomVariable::value()
{      
	double ret=0;
	
	if (first==1)
	{
		ret = rng_->gamma_rv(alpha_, 1);
		first = 0;
	}
	else ret = (rng_->beta(alpha_*rho_, alpha_*(1-rho_))*x_old + rng_->gamma_rv((1-rho_)*alpha_,1));
	x_old=ret;
